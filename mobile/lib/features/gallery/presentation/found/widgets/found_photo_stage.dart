...
/// (right), photographer + share/download bar (bottom).
///
      ...
      child: Center(
        child: ResolvedAspect(
          ...
          builder: (context, aspect) => LayoutBuilder(
            // The stage's own size, which decides whether this photo is short
            // enough to be worth lifting — see [liftedAspect]. AspectRatio
            // alone cannot know: it is handed a shape and fits it.
            builder: (context, constraints) {
              final boxAspect = liftedAspect(
                photoAspect: aspect,
                availableWidth: constraints.maxWidth,
                availableHeight: constraints.maxHeight,
              );
              final lifted = boxAspect != aspect;
                    ...
                    if (photo.isVideo)
                      ...
                      ZoomableArea(
                        ...
                        child: JpergImage(
                          ...
                          //
                          // A lifted box is deliberately *not* the photo's shape,
                          // and there `contain` would put the letterbox back inside
                          // the taller box and gain nothing — so it fills instead,
                    ...
                    Positioned(
                      ...
                      child: CommentSheetHide(
                        child: FoundPhotoMetaBar(
                            photo: photo, purchaseGated: purchaseGated),
                      ),
                    ),
                  ],
                ),
